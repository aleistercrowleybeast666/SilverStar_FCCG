...
串口维护协议0.0由System Console解析，UART Console Adapter只处理字节链路。IMU/GNSS/BARO/MAG/ATTITUDE/TELEMETRY/POWER使用`<CAPABILITY> <INSTANCE> <COMMAND>`，系统和算法模块保持无实例号；旧无实例能力命令不保留。Physical Device与Capability Endpoint通过Generated descriptor中的`physical_device_id`、`device_class`和类别内`instance_id`区分；同一JY901B source index的IMU、BARO和ATTITUDE端点共享一个物理ID及独立于其他JY901B的context。Generated facade对每个能力提供Count与静态`switch(instance_id)` direct binding，不存在实例返回`NOT_PRESENT`，不得回退到0。JY901B、NEO-M9N和SX1281同插件重复资格为1且最多4个静态实例；这只加入pre-start IMU锁定、GNSS liveness及AIR本地TX-timeout的最小单向切换，不等于Voting或Multi-EKF。`STATUS`给出运行与健康摘要，不新增独立`HEALTH`命令；`IO CLEAR`按物理ID建立共享显示基线，不清底层累计计数、Parser、Platform ring buffer或健康状态。SerialTask不直接执行设备事务、状态转换或功率动作。
