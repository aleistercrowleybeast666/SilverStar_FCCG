...
old/new instance必须均对应现有descriptor，未使用位保持0。IMU在Calibration/Alignment前选中备用时先写reason 5，保证该EVENT早于对应`CALIBRATION_RESULT`；锁定后及飞行中不再自动切IMU。GNSS基础liveness切换使用reason 6，fresh no-fix不触发。AIR在连续10次真实本地TX timeout后真实切换才使用reason 7，初始化跳过失败primary使用reason 8；无backup而继续重试不伪造source-change。该事件复用既有12-byte EVENT payload，不改变飞行日志格式0.0文件头、Record header、CRC、endianness或容器magic。
