...
Repeated official devices use deterministic IDs such as `imu0`, `imu1`, `gnss0`, `gnss1`,
`telemetry0`, and `telemetry1`. The project persists each physical instance and each
`<instance>:<requirement>` resource assignment independently; it does not persist mutable runtime
health or the active failover position. The configured capability source is the initial primary.
Generation fixes the remaining compatible instances in stable project order as the one-way backup
chain. Removing an instance runs the normal candidate reconcile pipeline, clearing dangling
assignments, overrides, protocol candidates, logging availability, and decoder semantics before the
...
The Board fixed mapping is plugin-owned rather than duplicated into `SilverStar.ssproject`.
`.fccg/hardware-preparation.json` and `.fccg/ownership.json` therefore store a deterministic
`resource_binding_fingerprint`. For a verified Board it covers Board ID/version/manifest hash,
logical ID, physical alias, resolved handle/port/pin/channel and physical pin, plus the renderer
contract. Readiness recomputes it from the installed Board snapshot, so changed connections,
generated defines, snapshot facts, manifest, or renderer contract makes the project stale. Custom
hardware uses the same marker with its imported resources and `logical_index`; its persisted manual
