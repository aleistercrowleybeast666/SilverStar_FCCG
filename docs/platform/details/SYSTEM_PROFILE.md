...
| 入口 | 职责 |
|---|---|
| `System/User/system_user_config.h` | 采样率、导航/KF、日志、诊断与通用策略默认；平台版本由FCCG app/version.py派生 |
...
| `Targets/<target>/target.mk` | MCU/Platform构建参数；源列表由Generated/project_sources.mk提供 |
| `Targets/<target>/Inc/target_system_config.h` | 目标能力enable、Required/Optional与Device qualification映射 |

当前不存在`system_user_registry.h`。把具体设备名、头文件或入口宏写回System/User属于架构回退。

## 2. Target composition

当前生产验证限于`SilverStar_F407`/SS0.5。所选MCU/Platform manifest拥有Target Profile，项目中的`build.target_profile`仅为派生完整性锁；Device、Algorithm、FlightLogic和Protocol选择共同解析为唯一Source Graph，再生成Make/EIDE。设备能力由静态Adapter/facade提供，power/storage属于Device，indicator/mission action属于相应FlightLogic实现；Board仅保留物理映射和约束。
