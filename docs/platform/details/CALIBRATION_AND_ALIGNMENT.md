...
本文是 SilverStar 0.0.12 的 Sensor Calibration、Initial Alignment 和任务初始姿态契约。坐标系定义以 [COORDINATE_FRAMES.md](COORDINATE_FRAMES.md) 为准，运行期导航以 [NAVIGATION_AND_ESTIMATION.md](NAVIGATION_AND_ESTIMATION.md) 为准。
...
`SystemCalibration`始终支持`NONE`，`ONE_FACE`和`SIX_FACE`按build procedure mask编入。输出保存在 `SystemCalibrationImuCorrection`：
...
NONE始终是合法单位校正能力；OneFace/SixFace仅在编入对应采样procedure时可用。空选的boot/Reset行为、有采样procedure时显式默认NONE、模式门禁与ACK以[共同Calibration契约](../../AIR_CALIBRATION_CONTRACT.md)为准。
