...
The current reference graph has 137 C sources and one startup assembly source. It includes only the selected Alignment/INS/Estimator/Landing strategies, all three independently selected Protocol Profiles, the FreeRTOS static kernel subset/CM4F port, and FCCG-generated C files. `Core/Src/sysmem.c` is excluded; both target memory and generated flight-configuration headers are forced includes, keeping deployment parameters identical across Make/EIDE.

Platform resource contributions are also resolved here. The MCU/Platform manifest declares a base graph plus per-resource backends and CubeMX-provider source candidates. An I²C, Classic-CAN, or PWM backend enters the graph only when a selected Device has an actual assignment of the matching inventory kind; required provider sources must exist in the Board/imported snapshot. Default SS0.5 has no such assignments, so no optional Platform backend and no unrelated I²C/CAN HAL source is emitted to Make, EIDE, or VS Code.
